    ...
    public void agregarLicencia(Label labIDEmpleadoCrear, Label labDiasDisponiblesCrear, DatePicker dpFechaInicioCrear, DatePicker dpFechaFinCrear, ComboBox<String> cbTipoLicenciaCrear, TextField textDescripcionLicenciaCrear, Label labLimpiarCamposCrear) throws ParseException, SQLException {
        ...
        if (-Cant_Dias >= 0) {
            if (-Cant_Dias2 > 0) {
                    ...
                    if (resultado2.equals("YES")) {
                        if(resultado3.equals("YES")){
                            ...
                            try {
                                ...
                                alerta.setTitle("Crear licencia");
                        ...
                        Alert alerta = new Alert(Alert.AlertType.ERROR);
                        alerta.setTitle("Error - Fecha");
                        alerta.setContentText("El empleado ya tiene una licencia entre la fecha de inicio y fin que ha seleccionado.");
                        Image icon = new Image(Objects.requireNonNull(getClass().getResourceAsStream("/imagenes/icono_Alerta.png")));
                        Stage stage = (Stage) alerta.getDialogPane().getScene().getWindow();
                        stage.getIcons().add(icon);
                        alerta.showAndWait();
                ...
                Alert alerta = new Alert(Alert.AlertType.ERROR);
                alerta.setTitle("Error - Fecha");
                alerta.setContentText("La fecha de inicio seleccionada debe ser un dia después de la fecha actual.");
                Image icon = new Image(Objects.requireNonNull(getClass().getResourceAsStream("/imagenes/icono_Alerta.png")));
                Stage stage = (Stage) alerta.getDialogPane().getScene().getWindow();
                stage.getIcons().add(icon);
                alerta.showAndWait();
